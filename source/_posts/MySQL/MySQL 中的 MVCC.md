...
date: 2025-08-26
description: 这篇博客详细介绍了MySQL中MVCC（多版本并发控制）的实现机制及其在不同事务隔离级别中的应用。MVCC通过维护数据的多个版本、结合undo log和readview来实现事务的并发控制，从而在不加锁或减少加锁的情况下提高数据库的并发性能。博客还解释了四种事务隔离级别（读未提交、读已提交、可重复读、串行化）的具体实现方式，重点阐述了可重复读和读已提交如何通过MVCC和readview机制来避免脏读和不可重复读问题，并进一步探讨了可重复读隔离级别下幻读问题的产生及解决方案，包括快照读和当前读的配合使用以及加锁策略等。
tags:
  - MySQL
  - MVCC
categories:
