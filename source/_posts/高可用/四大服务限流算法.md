...
date: 2025-08-26
top: 120
description: 这篇博客详细介绍了服务限流的概念、重要性以及四种常见的限流算法：固定窗口、滑动窗口、漏桶和令牌桶。博客通过生活化例子解释了限流的必要性，包括防止系统过载、保证服务质量、抵御攻击等。固定窗口算法简单但存在临界问题，滑动窗口算法通过更细粒度的时间划分改善了这一问题，漏桶算法以恒定速率处理请求但无法应对突发流量，而令牌桶算法则结合了平均速率控制和突发流量处理的优势，是实际应用中最常用的方案。博客还讨论了限流对象的选择（如IP、业务ID等）以及单机限流（基于Guava的RateLimiter）和分布式限流（基于Redisson）的实现方式，为构建高可用系统提供了全面的技术指导。
tags:
  - 分布式限流
  - 固定窗口
  - 滑动窗口
